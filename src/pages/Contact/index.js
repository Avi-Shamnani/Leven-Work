...
function Contact() {
  ...
  const handleCountryCodeChange = (e) => {
    ...
    const country = countries.find(
      (country) =>
        country.idd && country.idd.root && countryCode === (country.idd.root + (country.idd.suffixes ? country.idd.suffixes[0] : ''))
    );
        ...
        <form action="#" className="form">
          ...
          <div className="input-box">
            ...
            <div className="phone-input flex">
              ...
              <select id="country-code" value={selectedCountryCode} onChange={handleCountryCodeChange}>
                <option value="">Select Country Code</option>
                {countries.map((country, index) => (
                  country.idd && country.idd.root ? (
                    <option key={index} value={country.idd.root + (country.idd.suffixes[0] || '')}>
              ...
              {/* Display Flag */}
              {flagUrl && <img  src={flagUrl} alt="Country flag" style={{ width: '50px' ,height: '40px', marginLeft: '5px' }} />}
            </div>
            <input
  className="inline"
  type="tel"
...
/>
          ...
          <div className="input-box">
            ...
            <input type="text" placeholder="What Business do you have?" required />
          </div>
          ...
          {/* Message */}
          <div className="input-box">
            <textarea id="message" name="message" rows="4" placeholder="Message" style={{ textAlign: 'center' }} required></textarea>
          </div>

          {/* Submit Button */}
          <button id="submit-btn" type="submit">Connect With Us Now!</button>
        </form>
      </section>
      </div>
    </Layout>
