...
function ExpertLayout() {
  const works = [
    ...
    },
    {
      ...
      image: "https://images.pexels.com/photos/3782226/pexels-photo-3782226.jpeg?auto=compress&cs=tinysrgb&w=600",
      description: `
  ...
  return (
    <div className="flex flex-wrap justify-center bg-[#D6EFFF] text-white p-10 rounded-xl gap-8 mb-0">
      ...
      {works.map((work, index) => (
        <div key={index} className="w-full sm:w-80 md:w-[500px] lg:w-[500px] h-[800px] m-4 p-4 bg-[#000] rounded-2xl flex flex-col relative">
          <div className="w-full h-[500px] mb-4">
            <img src={work.image} alt={work.title} className="w-full h-full object-cover rounded-xl" />
          </div>
          <div className="w-full flex flex-col p-4 flex-grow">
            ...
            <p className="text-base md:text-xl text-white mb-8" dangerouslySetInnerHTML={{ __html: work.description }} />
            <a href="/contact" className="absolute bottom-4 right-4 text-2xl text-blue-400 border-red-600">
              {work.linkTitle}
            </a>
          </div>
        </div>
